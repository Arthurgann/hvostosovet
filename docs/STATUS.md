...
### LLM / Prompts — принципы и правила изменений

- Промпты режимов `care`, `vaccines`, `emergency` считаются **стабилизированными**.
- Используется версия **v2.2-balanced** как базовая и зафиксированная.
- Автотесты (`prompt-eval`) используются как **сигнальная сетка**, а не критерий качества.
- Изменения промптов допускаются **только после подтверждения проблемы в живых диалогах**
  и фиксируются отдельным этапом.

### LLM / Prompts (текущий статус)

- Текстовые промпты (`care / vaccines / emergency`) переработаны, протестированы и зафиксированы.
- Vision использует отдельные system-промпты по режимам.
- Подбор моделей завершён, конфигурация зафиксирована:
  - Text: Gemini (OpenRouter)(free режим - google/gemini-2.0-flash-001, pro режим - google/gemini-2.5-flash), fallback — `gpt-4.1-mini`;
  - Vision: `openai/gpt-4o-mini` (OpenRouter) (запасной - google/gemini-3-flash-preview).
- Этап экспериментов с моделями и промптами завершён.
- Текущий фокус проекта — UX, сценарии использования и прод-стабильность.
...
- Возможность включения `parse_mode` зафиксирована как **отложенное решение**,
  ...
  ### Vision (фото) — текущее состояние

- Поведение Vision считается **стабилизированным (v1)**:
  - модель ориентируется на текущее изображение;
  - не переносит описание предыдущих фото внутри одной session;
  - корректно обрабатывает неинформативные изображения
    (размыто / темно / нет деталей) без шаблонных отказов.
- Ответы по прикладным вопросам с фото намеренно краткие;
  основной фокус — рекомендации по сути вопроса.
- Нерелевантные фото обрабатываются мягко, без жёстких отказов —
  это осознанное продуктовое решение.

  ### LLM — выбор провайдера и моделей (Text)

- Добавлен ручной переключатель провайдера для **текстовых ответов**:
  - `TEXT_PROVIDER=openai | openrouter`.
- Для текста поддерживается разделение моделей по планам:
  - `*_TEXT_MODEL_FREE`;
  - `*_TEXT_MODEL_PRO`.
- Vision-поведение **не затронуто**:
  - анализ изображений всегда выполняется через **OpenRouter**
    с моделью из `OPENROUTER_VISION_MODEL`.
- Добавлены guard-проверки конфигурации провайдера:
  - при отсутствии ключа возвращается понятная ошибка `503`
    (`openai_not_configured` / `openrouter_not_configured`).
- Текущая схема признана стабильной и пригодной для:
  - размещения на зарубежном сервере (OpenAI);
  - последующего переноса на RU-сервер (OpenRouter).
