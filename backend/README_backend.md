...
- `GET /v1/pets/active?telegram_user_id=...` — возвращает активного питомца/профиль (используется ботом в “⭐ Мой питомец”)
...
Поддержка профиля питомца:
- `pet_profile` может быть передан в payload (имеет приоритет над БД);
- если `pet_profile` не передан — backend подтягивает профиль активного питомца
  из `pets.profile` и добавляет его в prompt (effective pet profile).
